...
class Particle(private var _rotation: Rotation, private var _position: Vect, val particleType: ParticleType, var _velocity: Vect = Vect.ZERO)(implicit val universe: Universe) {
  ...
  val DELTA_TIME = universe.resolution
  ...
  def collide (that: Particle): List[Force] = {
    ...
    val u2t: Double = u2 dot tangent

    val v1t = u1t
    val v2t = u2t

    val v1n = ( ((m1-m2) * u1n) + (2 * m2 * u2n) ) / (m1 + m2)
    val v2n = ( ((m2-m1) * u2n) + (2 * m1 * u1n) ) / (m1 + m2)

    val vect_v1n = normal * v1n
    val vect_v1t = tangent * v1t

    val vect_v2n = normal * v2n
    val vect_v2t = tangent * v2t

    val v2 = vect_v2n + vect_v2t
    val v1 = vect_v1n + vect_v1t


    val totalMomentumChange = ((v1 * m1 + v2 * m2) - (u1 * m1 + u2 * m2)).length
    if (totalMomentumChange > 0.001) println(s"Collision momentum change = $totalMomentumChange")

    val impulse1 = ((v1 * m1) - (u1 * m1)).length
    val impulse2 = ((v2 * m2) - (u2 * m2)).length


    if (impulse1 < 0.0000001) {
      List()
    } else {

      val forces = List(
        Force(this, impulse1, (this.position - that.position).theta, alwaysDraw = true),
        Force(that, impulse2, (that.position - this.position).theta, alwaysDraw = true)
      )
